...
def _concat_audio(
    ...
    # Backward compatibility: older callers supplied plain float scene
    # durations. Normalize them into the canonical scene-timing contract so
    # audio concatenation remains exact for both interfaces.
    ...
    scene_durations = [
        max(0.0, float(item.get("duration", MIN_SCENE_DURATION)))
        ...
        with wave.open(
            ...
            for index, scene_duration in enumerate(scene_durations):
                ...
                try:
                    ...
                    narration_start = float(normalized_timings[index].get("narration_start", MIN_PRE_ROLL))
