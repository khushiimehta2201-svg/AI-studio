"""Lightweight trainer/trainee portal state for one continuous tutorial per PDF."""
import json
...
def list_all_tutorials() -> List[Dict[str, Any]]:
    ...
    for job_dir in sorted(JOBS_DIR.iterdir()):
        ...
        out.append({
            ...
            "action_bullets": [s.get("title") for s in steps if s.get("title")],
            "action_count": len(steps),
            "duration": plan.get("video_duration", 0),
            "overview": plan.get("overview") or plan.get("description") or "Guided software workflow tutorial.",
...
def set_published(vid: str, published: bool) -> None:
    lib = load_library(); _entry(lib, vid)["published"] = bool(published); _save_json(LIBRARY_PATH, lib)
...
def set_prerequisites(vid: str, prerequisite_ids: List[str]) -> List[str]:
    lib = load_library(); warnings=[]; safe=[]
    ...
    _entry(lib, vid)["prerequisites"] = safe; _save_json(LIBRARY_PATH, lib); return warnings
...
def mark_watched(trainee_id: str, vid: str) -> None:
    progress = load_progress(); watched = progress.setdefault(trainee_id, [])
...
def get_missing_prerequisites(trainee_id: str, vid: str, lib=None, progress=None) -> List[str]:
    lib = lib if lib is not None else load_library(); progress = progress if progress is not None else load_progress()
